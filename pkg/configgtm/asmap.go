...
type ASMaps interface {
	...
	NewDefaultDatacenter(context.Context, *AsMap, int) *DatacenterBase
...
func (asm *AsMap) Validate() error {
	...
	if len(asm.DefaultDatacenter) == nil {
...
// Instantiate new Default Datacenter Struct
func (p *gtm) NewDefaultDatacenter(ctx context.Context, as *AsMap, dcID int) *DatacenterBase {

	logger := p.Log(ctx)
	logger.Debug("NewDefaultDatacenter")
...
func (p *gtm) UpdateAsMap(ctx context.Context, as *AsMap, domainName string) (*ResponseStatus, error) {
	...
	// common code
	stat, err := as.save(ctx, domainName)
	if err != nil {
		return nil, err
	}
...
func (as *AsMap) save(ctx context.Context, domainName string) (*AsMapResponse, error) {

	if err := as.Validate(); err != nil {
		logger.Errorf("AsMap validation failed. %w", err)
		return nil
	}
